...
toyexample_path = os.path.dirname(os.path.realpath(__file__))
dir_imgs = os.path.join(toyexample_path, "images")
dir_gts = os.path.join(toyexample_path, "gts_vocpascal_format")
dir_dets = os.path.join(toyexample_path, "dets_classname_abs_xywh")
dir_outputs = os.path.join(toyexample_path, "images_with_bbs")
...
def draw_bb_into_image(image, bounding_box, color, thickness, label=None):
    ...
    cv2.rectangle(
        image,
        (int(bb_coordinates[0]), int(bb_coordinates[1])),
        (int(bb_coordinates[2]), int(bb_coordinates[3])),
        (b, g, r),
        thickness,
    )
    # Add label
...
red_color = [255, 0, 0]
imgfiles = general_utils.get_files_recursively(dir_imgs)
print(f"Found {len(imgfiles)} images")
for img_file in imgfiles:
    # Get corresponding GT bounding boxes
    gt_annotation_file = general_utils.find_file(
        dir_gts, os.path.basename(img_file), match_extension=False
    )
    if gt_annotation_file is None:
        continue
    gt_bbs = converter.vocpascal2bb(gt_annotation_file)
    # Get corresponding detections bounding boxes
    det_annotation_file = general_utils.find_file(
        dir_dets, os.path.basename(img_file), match_extension=False
    )
    ...
    for bb in gt_bbs:
        ...
        image = draw_bb_into_image(
            image, bb, green_color, thickness=6, label=bb.get_class_id()
        )
    for bb in det_bbs:
        ...
        bb._x2 = min(bb._x2, img_w - 3)
        image = draw_bb_into_image(
            image, bb, red_color, thickness=6, label=bb.get_class_id()
        )
