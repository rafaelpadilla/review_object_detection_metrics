...
class Main_Dialog(QMainWindow, Main_UI):
    ...
    def load_annotations_gt(self):
        ...
        if self.rad_gt_format_coco_json.isChecked():
            ...
            ret = converter.openimage2bb(
                self.dir_annotations_gt, self.dir_images_gt, BBType.GROUND_TRUTH
            )
    ...
    def validate_det_choices(self):
        ...
        if (
            self.rad_det_ci_format_text_yolo_rel.isChecked()
        ...
        if (
            ...
            if self.filepath_classes_det is not None and os.path.isfile(
    ...
    def load_annotations_det(self):
        ...
        if self.rad_det_format_coco_json.isChecked():
            ...
            self.rad_det_ci_format_text_xyx2y2_abs.isChecked()
            ...
            self.rad_det_ci_format_text_xywh_abs.isChecked()
    ...
    def btn_gt_statistics_clicked(self):
        ...
        if self.rad_gt_format_yolo_text.isChecked():
            if (
                self.filepath_classes_gt is None
                or os.path.isfile(self.filepath_classes_gt) is False
            ):
                self.show_popup(
        ...
        self.dialog_statistics.show_dialog(
            BBType.GROUND_TRUTH, gt_annotations, None, self.dir_images_gt
        )
    ...
    def btn_gt_classes_clicked(self):
        filepath = QFileDialog.getOpenFileName(
            self,
            "Choose a file with a list of classes",
    ...
    def btn_gt_images_dir_clicked(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Choose directory with ground truth images", txt
        )
        if directory != "":
    ...
    def btn_statistics_det_clicked(self):
        ...
        self.dialog_statistics.show_dialog(
            BBType.DETECTED, gt_annotations, det_annotations, self.dir_images_gt
        )
    ...
    def btn_output_dir_clicked(self):
        ...
        directory = QFileDialog.getExistingDirectory(
            self, "Choose directory to save the results", txt
        )
        if os.path.isdir(directory):
    ...
    def btn_run_clicked(self):
        if (
            self.dir_save_results is None
            or os.path.isdir(self.dir_save_results) is False
        ):
            self.show_popup(
        ...
        if (
            self.chb_metric_AP_coco.isChecked()
