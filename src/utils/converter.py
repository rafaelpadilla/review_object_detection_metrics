    ...
    for file_path in annotation_files:
        ...
        for image_info in ET.parse(file_path).iter("image"):
            ...
            for box_info in image_info.iter("box"):
                x1, y1, x2, y2 = (
                    float(box_info.attrib["xtl"]),
                    float(box_info.attrib["ytl"]),
                    float(box_info.attrib["xbr"]),
                    float(box_info.attrib["ybr"]),
                )
...
def openimage2bb(annotations_path, images_dir, bb_type=BBType.GROUND_TRUTH):
    ...
    for file_path in annotation_files:
        ...
        for i, row in csv.iterrows():
            ...
            img_size = (
                images_shapes[image_file]["width"],
            ...
            if bb_type == BBType.DETECTED and confidence is None:
                ...
                return ret
            bb = BoundingBox(
                image_name=general_utils.get_file_name_only(row["ImageID"]),
...
def text2bb(
    ...
    for file_path in annotation_files:
        if type_coordinates == CoordinatesType.ABSOLUTE:
            if (
                bb_type == BBType.GROUND_TRUTH
                and not validations.is_absolute_text_format(
            ...
            if (
                bb_type == BBType.GROUND_TRUTH
                and not validations.is_relative_text_format(
        ...
        with open(file_path, "r") as f:
            ...
            if type_coordinates == CoordinatesType.RELATIVE:
                img_path = general_utils.find_image_file(
                    img_dir, img_filename + file_suffix
                )
...
def df2labelme(symbolDict, dir_image):
    try:
        ...
        if (
            "XMin" in symbolDict.columns
            and "YMin" in symbolDict.columns
            and "XMax" in symbolDict.columns
            and "YMax" in symbolDict.columns
        ):
            ...
            symbolDict = (
                symbolDict.apply(
                    lambda x: x[["label", "points", "shape_type", "group_id"]].to_dict(
                        "records"
                    )
                )
                .reset_index()
