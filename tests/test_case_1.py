...
def test_case_1():
    ...
    for idx, iou in enumerate(testing_ious):
        results = get_pascalvoc_metrics(gts, dets, iou_threshold=iou, method=MethodAveragePrecision.ELEVEN_POINT_INTERPOLATION)
    ...
    expected_APs = {'object': {
                        ...
                        0.75: 0.0 }}
    for idx, iou in enumerate(testing_ious):
        results = get_pascalvoc_metrics(gts, dets, iou_threshold=iou, method=MethodAveragePrecision.EVERY_POINT_INTERPOLATION)
        for c, res in results.items():
            assert isclose(expected_APs[c][iou], res['AP'])
